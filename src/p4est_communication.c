...
struct p4est_transfer_internal_s
{
  ...
  p4est_transfer_meta_t *resp, *dup;
...
p4est_queries_context_new (sc_array_t * queries)
{
  ...
  c->resp_buffers = c->resp_receivers = c->resp_ratios = c->resp_senders =
    NULL;
  c->dup_buffers = c->dup_receivers = c->dup_ratios = c->dup_senders = NULL;
                       ...
                       void *query_index)
{
  ...
  p4est_transfer_meta_t *dup = internal->dup;
...
exchange_ratios (p4est_transfer_meta_t *meta)
{
  ...
  int                 mpiret;
  size_t              is, ir;
  size_t              num_senders, num_receivers;
  ...
  /* receive ratios of all processes this process will send to */
  num_receivers = meta->receivers->elem_count;
  meta->recvs_ratios = sc_array_new_count (sizeof (double), num_receivers);
  recv_reqs = P4EST_ALLOC (sc_MPI_Request, num_receivers);
...
p4est_transfer_search (p4est_t *p4est, p4est_queries_context_t *c,
                       p4est_intersect_t intersect_fn)
{
  p4est_transfer_internal_t *ti;

  /* Call transfer search begin */
  ti = p4est_transfer_search_begin (p4est, c, intersect_fn);

  return p4est_transfer_search_end (ti);
}

int
p4est_transfer_search_ext (p4est_t *p4est, p4est_queries_context_t *c,
...
p4est_transfer_search_internal_begin (p4est_transfer_internal_t *internal)
{
  ...
  /* note: an error is recorded here if p is attempting to send more than
     INT_MAX bytes in an dup or resp message to another process. We defer
