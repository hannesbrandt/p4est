 ...
 */
typedef size_t      (*p8est_query_weight_t) (void *query, void *user);
 ...
 *
 * Calling \ref p8est_transfer_search performs the transfer of queries and
 ...
 * \ref p8est_transfer_search. For example, query coordinates could
 ...
 *
 * During the transfer of queries in \ref p8est_transfer_search the \a queries
 * array is destroyed and reallocated. Thus users should not maintain pointers
 * to it or its contents.
 */
typedef struct p8est_queries_context
{
   ...
   * \ref p8est_transfer_search is called.  These queries are stored in the
   ...
   * positions of \c queries. This is only relevant if \ref
   * p8est_transfer_search is called with the \a save_outside option.
   */
...
/** Initialize a p8est_queries_context_t for a subsequent transfer search.
 ...
 * \ref p8est_queries_context. Users should take care to maintain this
 ...
 * \param [in] save_outside If true then queries outside of the \a p8est are
...
void                p8est_queries_context_destroy (p8est_queries_context_t *c);
